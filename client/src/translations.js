const translations = {
  en: {
    appTitle: 'Next Gen E-Commerce',
    username: 'Username',
    password: 'Password',
    login: 'Login',
    newUser: 'New User?',
    registerLink: 'Register',
    register: 'Register',
    firstName: 'First Name',
    lastName: 'Last Name',
    email: 'Email',
    alreadyHaveAccount: 'Already have an account?',
    loginLink: 'Login',
    userRegistered: (userName) => `User registered: ${userName}`,
    welcome: (userName) => `Welcome ${userName}`,
    invalidCredentials: 'Invalid username or password',
    hello: 'Hello,',
    myAccount: 'My Account',
    orders: 'Orders',
    products: 'Products',
    adminPanel: 'Admin Panel',
    logOut: 'Log Out',
    loggedOut: 'Logged out!',
    helloAdmin: 'Hello, Admin',
    account: 'Account',
    categories: 'Categories',
    customers: 'Customers',
    statistics: 'Statistics',
    category: 'Category:',
    all: 'All',
    price: 'Price:',
    clear: 'Clear',
    inStock: 'In stock:',
    bought: 'Bought',
    total: 'Total:',
    items: 'Items:',
    order: 'Order',
    clearCart: 'Clear',
    orderPlaced: 'Order placed!',
    ordersTitle: 'Orders',
    titleCol: 'Title',
    qtyCol: 'Qty',
    totalCol: 'Total',
    dateCol: 'Date',
    save: 'Save',
    allowOthers: 'Allow others see my orders:',
    userUpdated: 'User Updated!',
    categoriesTitle: 'Categories',
    update: 'Update',
    remove: 'Remove',
    newName: 'New Name',
    addNewCategory: 'Add new category',
    addNew: 'Add New',
    categoryAdded: 'Category added successfully',
    categoryRemoved: 'Category removed successfully',
    categoryUpdated: 'Category updated successfully',
    errorAddingCategory: 'Error adding category',
    errorRemovingCategory: 'Error removing category',
    errorUpdatingCategory: 'Error updating category',
    titleLabel: 'Title:',
    quantityLabel: 'Quantity:',
    priceLabel: 'Price:',
    categoryLabel: 'Category:',
    descriptionLabel: 'Description:',
    pictureLabel: 'Picture:',
    nameCol: 'name',
    qtySmall: 'qty',
    dateSmall: 'date',
    fillRequired: 'Please fill all required fields',
    productUpdated: 'Product updated successfully',
    errorUpdatingProduct: 'Error updating product',
    productAdded: (title) => `Product "${title}" added successfully`,
    errorAddingProduct: 'Error adding product',
    customersTitle: 'Customers',
    fullName: 'Full Name',
    joinedAt: 'Joined at',
    productsBought: 'Products Bought',
    product: 'Product',
    check: 'Check',
    totalSoldProducts: 'Total Sold Products',
    selectUser: 'Select User',
    qtyLabel: 'Qty',
    langToggle: 'עברית',
    currency: '₪',
    addToCart: 'Add to Cart',
    inStockStatus: 'In stock',
    outOfStock: 'Out of stock',
    uploadError: 'Error uploading image',
    productDeleted: 'Product deleted successfully',
    errorDeletingProduct: 'Error deleting product',
    phoneNumber: 'Phone Number',
    address: 'Address',
    city: 'City',
    postalCode: 'Postal Code',
    SmartHomeTech: 'Smart Home Light for your home',
    productCatalog: 'Product Catalog',
    ourProducts: 'Our Products',
    smartGadgets: 'Smart Gadgets for your home',
    SurprisingPrices: 'Surprising Prices',
    addedToCart: 'Added to cart',
    viewCart: 'View cart',
    retry: 'Try again',
    errorLoadingProducts: "We couldn't load the products. Check your connection and try again.",
    errorLoadingProduct: "We couldn't load the product. Check your connection and try again.",
    errorLoadingOrders: "We couldn't load your orders. Check your connection and try again.",
    errorSubmittingOrder: 'Order submission failed. Check your connection and press the payment button again.',
    fillShippingDetails: 'Please fill in all shipping details before completing the payment.',
    navigationMenu: 'Navigation menu',
    paymentSuccessMessage: 'Payment received! Your order is on its way',
    paymentFailedMessage: 'Payment was not completed, please try again',
    viewMyOrders: 'View My Orders',
    backToPayment: 'Back to payment',
    footerTagline: 'Smart gadgets for your home',
    footerQuickLinks: 'Quick Links',
    footerContactUs: 'Contact Us',
    footerWhatsapp: 'WhatsApp',
    landingHeroTitle: 'Smart Tech for Your Home',
    landingHeroSub: 'Smart lighting, gadgets and accessories for the modern home',
    landingHeroCta: 'Shop Now',
    landingTrustDelivery: 'Fast Delivery',
    landingTrustDeliverySub: 'Quick shipping on all orders',
    landingTrustWarranty: 'Full Warranty',
    landingTrustWarrantySub: 'Warranty on every product',
    landingTrustPayment: 'Secure Payment',
    landingTrustPaymentSub: 'Transactions secured via Cardcom',
    landingTrustSupport: 'Customer Support',
    landingTrustSupportSub: 'Here for you whenever you need',
    landingTopCategories: 'Top Categories',
    landingFeaturedProducts: 'Featured Products',
    landingViewAll: 'See All Products',
    landingCtaTitle: 'Ready to upgrade your home?',
    landingCtaSub: 'Discover our full collection of smart gadgets',
    landingCtaBtn: 'Browse the Catalog',
    landingNoProducts: 'No products to display yet',
  },
  he: {
    address: 'כתובת',
    city: 'עיר',
    postalCode: 'מיקוד',
    phoneNumber: 'מספר טלפון',
    appTitle: 'חנות דור הבא',
    username: 'שם משתמש',
    password: 'סיסמה',
    login: 'כניסה',
    newUser: 'משתמש חדש?',
    registerLink: 'הירשם',
    register: 'הרשמה',
    firstName: 'שם פרטי',
    lastName: 'שם משפחה',
    email: 'אימייל',
    alreadyHaveAccount: 'כבר יש לך חשבון?',
    loginLink: 'התחבר',
    userRegistered: (userName) => `המשתמש נרשם: ${userName}`,
    welcome: (userName) => `ברוך הבא ${userName}`,
    invalidCredentials: 'שם משתמש או סיסמה שגויים',
    hello: 'שלום,',
    myAccount: 'החשבון שלי',
    orders: 'הזמנות',
    products: 'מוצרים',
    adminPanel: 'פאנל ניהול',
    logOut: 'התנתק',
    loggedOut: 'התנתקת!',
    helloAdmin: 'שלום, מנהל',
    account: 'חשבון',
    categories: 'קטגוריות',
    customers: 'לקוחות',
    statistics: 'סטטיסטיקות',
    category: 'קטגוריה:',
    all: 'הכל',
    price: 'מחיר:',
    clear: 'נקה',
    inStock: 'במלאי:',
    bought: 'נקנה',
    total: 'סה"כ:',
    items: 'פריטים:',
    order: 'הזמן',
    clearCart: 'נקה',
    orderPlaced: 'ההזמנה בוצעה!',
    ordersTitle: 'הזמנות',
    titleCol: 'שם',
    qtyCol: 'כמות',
    totalCol: 'סה"כ',
    dateCol: 'תאריך',
    save: 'שמור',
    allowOthers: 'אפשר לאחרים לראות את ההזמנות שלי:',
    userUpdated: 'המשתמש עודכן!',
    categoriesTitle: 'קטגוריות',
    update: 'ערוך',
    remove: 'מחק',
    newName: 'שם חדש',
    addNewCategory: 'הוסף קטגוריה חדשה',
    addNew: 'הוסף',
    categoryAdded: 'הקטגוריה נוספה בהצלחה',
    categoryRemoved: 'הקטגוריה הוסרה בהצלחה',
    categoryUpdated: 'הקטגוריה עודכנה בהצלחה',
    errorAddingCategory: 'שגיאה בהוספת קטגוריה',
    errorRemovingCategory: 'שגיאה במחיקת קטגוריה',
    errorUpdatingCategory: 'שגיאה בעדכון קטגוריה',
    titleLabel: 'שם:',
    quantityLabel: 'כמות:',
    priceLabel: 'מחיר:',
    categoryLabel: 'קטגוריה:',
    descriptionLabel: 'תיאור:',
    pictureLabel: 'תמונה:',
    nameCol: 'שם',
    qtySmall: 'כמות',
    dateSmall: 'תאריך',
    fillRequired: 'אנא מלא את כל השדות הנדרשים',
    productUpdated: 'המוצר עודכן בהצלחה',
    errorUpdatingProduct: 'שגיאה בעדכון המוצר',
    productAdded: (title) => `המוצר "${title}" נוסף בהצלחה`,
    errorAddingProduct: 'שגיאה בהוספת המוצר',
    customersTitle: 'לקוחות',
    fullName: 'שם מלא',
    joinedAt: 'הצטרף ב',
    productsBought: 'מוצרים שנרכשו',
    product: 'מוצר',
    check: 'בדוק',
    totalSoldProducts: 'סך מוצרים שנמכרו',
    selectUser: 'בחר משתמש',
    qtyLabel: 'כמות',
    langToggle: 'EN',
    currency: '₪',
    addToCart: 'הוסף לעגלה',
    inStockStatus: 'במלאי',
    outOfStock: 'אזל המלאי',
    uploadError: 'שגיאה בהעלאת התמונה',
    productDeleted: 'המוצר נמחק בהצלחה',
    errorDeletingProduct: 'שגיאה במחיקת המוצר',
    SmartHomeTech: 'תאורה חכמה לבית שלך',
    productCatalog: 'לקטלוג מוצרים',
    ourProducts: 'המוצרים שלנו',
    addedToCart: 'נוסף לעגלה',
    viewCart: 'לעגלה',
    retry: 'נסה שוב',
    errorLoadingProducts: 'לא הצלחנו לטעון את המוצרים. בדוק את החיבור ונסה שוב.',
    errorLoadingProduct: 'לא הצלחנו לטעון את המוצר. בדוק את החיבור ונסה שוב.',
    errorLoadingOrders: 'לא הצלחנו לטעון את ההזמנות. בדוק את החיבור ונסה שוב.',
    errorSubmittingOrder: 'שליחת ההזמנה נכשלה. בדוק את החיבור ולחץ שוב על כפתור התשלום.',
    fillShippingDetails: 'יש למלא את כל פרטי המשלוח לפני ביצוע התשלום.',
    navigationMenu: 'תפריט ניווט',
    smartGadgets: 'גאדג\' טים ציוד חכם ואביזרים לבית המודרני',
    SurprisingPrices: 'במחירים מפתיעים',
    paymentSuccessMessage: 'התשלום התקבל! ההזמנה שלך בדרך אליך',
    paymentFailedMessage: 'התשלום לא הושלם, נסה שוב',
    viewMyOrders: 'צפה בהזמנות שלי',
    backToPayment: 'חזרה לתשלום',
    footerTagline: 'גאדג׳טים חכמים לבית שלך',
    footerQuickLinks: 'קישורים מהירים',
    footerContactUs: 'צור קשר',
    footerWhatsapp: 'וואטסאפ',
    landingHeroTitle: 'תאורה חכמה לבית שלך',
    landingHeroSub: 'תאורה חכמה, גאדג׳טים ואביזרים לבית המודרני',
    landingHeroCta: 'לקנייה עכשיו',
    landingTrustDelivery: 'משלוח מהיר',
    landingTrustDeliverySub: 'שליחה מהירה לכל הזמנה',
    landingTrustWarranty: 'אחריות מלאה',
    landingTrustWarrantySub: 'אחריות על כל מוצר',
    landingTrustPayment: 'תשלום מאובטח',
    landingTrustPaymentSub: 'עסקאות מוגנות דרך קארדקום',
    landingTrustSupport: 'שירות לקוחות',
    landingTrustSupportSub: 'כאן בשבילך בכל עת',
    landingTopCategories: 'קטגוריות פופולריות',
    landingFeaturedProducts: 'מוצרים מובחרים',
    landingViewAll: 'לכל המוצרים',
    landingCtaTitle: 'מוכן לשדרג את הבית?',
    landingCtaSub: 'גלה את כל הגאדג׳טים החכמים שלנו',
    landingCtaBtn: 'לקטלוג המוצרים',
    landingNoProducts: 'אין מוצרים להצגה עדיין',
  },
};

export default translations;

    ...
    {categories.map((category) => {
        ...
        <Card key={category._id} variant="outlined" component={Paper} sx={{ maxWidth: 250, mb: 2 }}  >
          <CardContent>
            ...
            >{t('update')}</Button>
            <Button onClick ={() => removeData(category._id)} size="small" sx={{ backgroundColor: 'lightgrey', color: 'black'}}>{t('remove')}</Button>
            </Box>

            {editingId === category._id && (
                <Box sx={{ mt: 1 }}>
                 <TextField placeholder={t('newName')}
                 InputProps={{
                    sx: {
                        height: 40,
                    }
                 ...
                 />
                 <Button onClick={() =>updateData(category._id)} size="small" sx={{ backgroundColor: 'lightgrey', color: 'black'}}>{t('save')}</Button>
                </Box>
            )}
          </CardContent>
        </Card>
...
/>
<Button onClick={newData} size="small" sx={{ backgroundColor: 'limegreen', color: 'black'}}>{t('addNew')}</Button>
  </Box>
</Box>
